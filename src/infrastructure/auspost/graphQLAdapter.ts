...
export function makeAusPostGraphQLAdapter(opts: { endpoint: string; apiKey: string }): AddressValidator {
  ...
  return {
    async validate(input: AddressInput): Promise<AddressValidation> {
      const { data } = await client.query<ValidateResult>({ query: QUERY, variables: input });
      if (!data?.validate) {
